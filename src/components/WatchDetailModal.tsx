...
export function WatchDetailModal({ watchId, onClose }: Props) {
  ...
  const stockLabel = (status: string) =>
    (status || 'unknown').split('_').map((w: string) => w.charAt(0).toUpperCase() + w.slice(1)).join(' ')
  ...
  return (
    <>
      ...
      {/* Side panel — full height, scrolls internally via overflow-y-auto on the whole panel */}
      <motion.div
        initial={{ x: '100%' }}
        animate={{ x: 0 }}
        exit={{ x: '100%' }}
        transition={{ type: 'spring', damping: 28, stiffness: 280 }}
        className="fixed top-0 right-0 bottom-0 z-50 w-full md:w-[78vw] lg:w-[58vw] max-w-3xl overflow-y-auto"
        style={{ background: '#0d0d0d', borderLeft: '1px solid #1f1f1f' }}
      >
        {/* Header — inline (not sticky, to avoid overflow conflict) */}
        ...
        <div className="p-6 space-y-8">
          {loading ? (
            ...
            <>
              ...
              <div className="flex flex-col sm:flex-row gap-5">
                ...
                <div className="flex-1 flex flex-col justify-center gap-2">
                  <div className="flex flex-wrap items-center gap-2">
                    ...
                    {watch.brand && <span className="text-sm text-text-muted">{watch.brand}</span>}
              ...
              {/* Price chart — explicit pixel height so Recharts can measure */}
              <div className="rounded-xl border border-[#1f1f1f] bg-black p-5">
                <PriceHistoryChart
                  watchId={watchId}
